...
| `/v1/pilot/*` + `/mcp/pilot`  | Partner API key, no crypto: invoice gate, verify, payment risk, company changes; payment waived, metered per partner via `pilot_call` events; keys in env `PILOT_KEYS` ([PILOT.md](./PILOT.md)) |
