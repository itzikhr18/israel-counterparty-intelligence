...
- The monthly count comes from the durable per-partner counters once the free Upstash database is set (PILOT.md §5): `GET /v1/pilot/usage?month=YYYY-MM` with the operator token lists every partner's month total by tool, and `npm run pilot:invoice -- --month YYYY-MM` turns it into invoice lines with VAT and the commission netted, using this price card by default. Without Upstash, only the `pilot_call` log events count, and Vercel keeps them briefly.
