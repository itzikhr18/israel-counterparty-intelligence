...
**Updated:** 2026-09-25 late morning Asia/Jerusalem  
...
| Pilot terms, one page, HE + EN (for the partner's product or procurement contact) | https://israel-counterparty-intelligence.vercel.app/pilot-terms.md               |
...
| Upstash Redis                                            | **Owner action, 5 min**       | Free database + two env vars. Now also the durable pilot-usage ledger for invoicing (`GET /v1/pilot/usage`), not only the settle record                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
4. **LinkedIn, after the holidays:** three Hebrew posts are ready to paste in [LINKEDIN_POSTS.md](./LINKEDIN_POSTS.md) (the ₪1,080 on a ₪6,000 invoice, how a non-developer shipped this, three checks before money leaves). **Post 1 published 25.09 13:19** (owner's call, ahead of the holiday schedule) with the link in the first comment; posts 2 and 3 stay on Tue 6.10 and Sun 11.10. Reply to every comment within hours. The only distribution channel that needs no cold email and no partner.
...
5. ~~Packaging so a partner can say yes without USDC~~ **Done 2026-09-25.** A partner key now unlocks the whole product: `POST /v1/pilot/{invoice-gate,verify,payment-risk,company-changes}` and all four tools on `/mcp/pilot`, payment waived, one key per partner in Vercel env `PILOT_KEYS` (JSON), usage metered by `pilot_call` events. Runbook: [PILOT.md](./PILOT.md). **Owner actions when a partner says yes:** issue a key (2 min), set `PILOT_KEYS`, redeploy. **Before the first billable month:** approve the ₪ price card in [PRICING_PROPOSAL.md](./PRICING_PROPOSAL.md) (proposal ready 2026-09-25), confirm with Atzmai (the chosen ₪ collection rail, see PRICING_PROPOSAL §3) that they issue allocation numbers on invoices above ₪5,000, and create the free Upstash database (5 minutes, [PILOT.md](./PILOT.md) §5) so usage is counted durably and `GET /v1/pilot/usage` gives the invoice numbers. **Pilot terms** for the partner's procurement are on one page at `/pilot-terms.md` (draft v0.1, HE + EN; owner to read once and approve). **Atzmai, registered 2026-09-25:** sign the contract and fill form 101 from their registration email (without them they cannot pay salary), and send the Gmail draft to `office@atzmai.co.il` (allocation number above ₪5,000, foreign-currency invoices, recurring monthly invoice).
6. **Create the free Upstash database and set `UPSTASH_REDIS_REST_URL` + `UPSTASH_REDIS_REST_TOKEN`** (5 minutes, owner). It now does two jobs: durable partner metering for invoicing (`GET /v1/pilot/usage`, allowance enforced across instances) and the durable first-settle record on `/health`.
...
| When (IDT)           | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-25 late am   | Hebrew free invoice check `/invoice-check/he` with a Hebrew result page (#28) and two one-click sample invoices flagged `sample: true` in telemetry (#29); pilot terms one-pager `/pilot-terms.md` (HE + EN, draft v0.1); Atzmai registration confirmed by email (contract + form 101 to sign); Gmail draft to Atzmai with three questions; Grow call prep updated with the demo; Yaki has not yet confirmed 10:30 or 12:00                                     |
| 2026-09-25 early am  | Partner channel shipped end to end (#21–#27): `PILOT_KEYS` multi-key, all four tools on `/v1/pilot/*` and `/mcp/pilot`, Upstash durable metering with `GET /v1/pilot/usage?month=`, `npm run pilot:invoice`, Hebrew partner page `/partner/he`, engineer's guide `/partner-integration.md`, ₪ price card proposal, Atzmai chosen as the ₪ collection rail, morning inbox routine spec; mcpservers.org listing approved, duplicate-entry removal request drafted |
